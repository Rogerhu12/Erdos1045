...
public import RiemannAuditShift
...
theorem exists_mapsTo_unitBall_injOn_deriv_ne_zero {U : Set ℂ} (hUo : IsOpen U)
  ...
  have hdf' : DifferentiableOn ℂ f U := fun z hz ↦
    (hdf z hz).hasDerivAt.hasDerivWithinAt.differentiableWithinAt
...
theorem UnitDisc.hasDerivWithinAt_shift_comp {f : ℂ → UnitDisc} {z f' : ℂ} {s : Set ℂ}
  ...
  simp only [coe_shift]
  apply ((hf.const_add _).fun_div ((hf.const_mul _).const_add _)
    (UnitDisc.shift_den_ne_zero _ _)).congr_deriv
  rw [← mul_conj']
  ring
...
theorem exist_map_unitDisc_injOn_norm_deriv_gt {U : Set ℂ} (hUo : IsOpen U)
  ...
  have hdg : ∀ z ∈ U, HasDerivAt (g · : ℂ → ℂ)
    ...
    intro z hz
    have hp : (2 : ℂ) * (UnitDisc.coe ∘ g) z ^ (2 - 1) ≠ 0 := by
      simpa using hg₀ z hz
    apply ((hasDerivAt_pow 2 _).of_comp_left
      (UnitDisc.continuous_coe.continuousAt.comp <| hgc.continuousAt <| hUo.mem_nhds hz)
      (UnitDisc.hasDerivAt_shift_comp _ <| (hdf.hasDerivAt <| hUo.mem_nhds hz)) hp
      (.of_forall fun x ↦ congr(UnitDisc.coe $(hgf x)))).congr_deriv
