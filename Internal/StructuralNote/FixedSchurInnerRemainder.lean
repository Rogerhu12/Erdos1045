...
/-! The actual remainder changes by order n^(-5/2) under a bounded Hamming
change at the same parameters in the fixed inner energy ball. -/

namespace StructuralNote.FixedSchurInnerRemainder
...
open FixedSchurChartCenterBounds FixedSchurHammingStability SolWordHamming
open scoped Topology

noncomputable section

def coefficient (B : ℝ) : ℝ :=
  32 * (3 * B * (30 + 3 * B) + 82 * (30 + 3 * B) ^ 2)

theorem coefficient_nonneg {B : ℝ} (hB : 0 ≤ B) : 0 ≤ coefficient B := by
  unfold coefficient
  positivity
...
theorem eventual_remainder_energy_bound (B : ℝ) (hB : 0 ≤ B) : ∀ᶠ m : ℕ in atTop,
  ...
  filter_upwards [eventually_chosen_inner_sizes B hB, eventual_coordinate_properties,
    eventual_quotient_properties, eventual_center_bounds] with m hinner hprops hquot hcenter
...
theorem eventual_remainder_hamming_bound (B : ℝ) (hB : 0 ≤ B) (s₀ : ℕ) :
    ∀ᶠ m : ℕ in atTop,
    ∀ (hm : 2 ≤ m) (s t : FiniteBox.SignPattern (by omega))
      (θ : Fin (2 * m) → ℝ) (v : Fin (2 * m) → ℂ), InDomain (by omega) θ v →
      pairEnergy (by omega) (fun j => (θ j : ℂ)) + pairEnergy (by omega) v ≤
  ...
  filter_upwards [eventual_remainder_energy_bound B hB, eventual_hamming_stability]
    with m hR hstable
...
theorem eventual_objective_hamming_bound (B : ℝ) (hB : 0 ≤ B) (s₀ : ℕ) :
    ∀ᶠ m : ℕ in atTop,
    ∀ (hm : 2 ≤ m) (s t : FiniteBox.SignPattern (by omega))
      (θ : Fin (2 * m) → ℝ) (v : Fin (2 * m) → ℂ), InDomain (by omega) θ v →
      pairEnergy (by omega) (fun j => (θ j : ℂ)) + pairEnergy (by omega) v ≤
      ...
      |(F (FixedSchurChart.configuration (by omega) s θ v) -
        ...
        (coefficient B * Real.sqrt (12800 * (s₀ : ℝ))) /
          ((2 * m : ℝ) ^ 2 * Real.sqrt (2 * m : ℝ)) := by
  filter_upwards [eventual_remainder_hamming_bound B hB s₀, eventual_coordinate_properties]
    with m hrem hprops
  intro hm s t θ v hdom henergy hham
...
end StructuralNote.FixedSchurInnerRemainder
